...
type Tab = "summaries" | "recommendations" | "stagnations";
const VALID_TABS: Tab[] = ["summaries", "recommendations", "stagnations"];
...
export default async function InsightsHubPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="flex flex-wrap gap-2">
        {CHIPS.map((c) => {
          ...
          return (
            <Link
              key={c.key}
              href={`/atourin/insights?tab=${c.key}`}
              className={`inline-flex h-9 items-center gap-1.5 rounded-full border px-3 text-xs font-bold transition ${
                isActive
                  ...
                  : "border-atr-outline bg-white text-atr-fg-muted hover:bg-atr-bg-soft hover:text-atr-fg"
              }`}
            >
      ...
      {tab === "summaries" && <SummariesPanel rows={data.summaries} />}
      {tab === "recommendations" && (
        <RecommendationsPanel rows={data.recommendations} />
      )}
      {tab === "stagnations" && <StagnationsPanel rows={data.stagnations} />}
    </div>
  );
}

function SummariesPanel({ rows }: { rows: InsightRow[] }) {
  if (rows.length === 0)
    return <EmptyCard message="Belum ada AI summary di-generate." />;
  return (
...
function RecommendationsPanel({ rows }: { rows: InsightRow[] }) {
  if (rows.length === 0)
    return <EmptyCard message="Belum ada AI recommendation di-generate." />;
  return (
    <ul className="space-y-2">
      {rows.map((r) => {
        const items =
          (
            r.content as {
              items?: Array<{ action: string; priority: number }>;
            }
          ).items ?? [];
        return (
          <li
...
function StagnationsPanel({ rows }: { rows: InsightRow[] }) {
  if (rows.length === 0)
    return <EmptyCard message="Belum ada desa stagnan terdeteksi 🎉" />;
  return (
    <ul className="space-y-2">
      {rows.map((r) => {
